    ...
    protected String getSelectByIdSQL() {
        return """
                SELECT
                    ...
                    ea.ability
                FROM PARANORMAL_ENTITY pe
                JOIN THREATS t ON t.id_threat = pe.id_entity
                LEFT JOIN THREATS_NAMES tn ON tn.id_threat = t.id_threat
                LEFT JOIN THREAT_ELEMENTS te ON te.id_threat = t.id_threat
                LEFT JOIN ELEMENTS e ON e.id_element = te.id_element
                LEFT JOIN ENTITY_ABILITY ea ON ea.id_entity = pe.id_entity
                WHERE pe.id_entity = ?
                """;
    }
        ...
        try (   Connection conn = ConnectionFactory.getConnection();
            ...
            while (rs.next()) {
                ...
                if (rs.getString("threat_names") != null) {
                    ...
                    System.out.println(rs.getString("ability"));
                if (rs.getString("ability") != null) {
                    entity.getAbilities().add(rs.getString("ability"));
                }
