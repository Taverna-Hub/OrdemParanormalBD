...
CREATE TRIGGER TRG_NEX_TRANCENDED_INSERT
    BEFORE INSERT ON AGENTS
    FOR EACH ROW
BEGIN
    SET NEW.transcended = VERIFY_NEX(NEW.nex);
END $$
...
CREATE TRIGGER TRG_NEX_TRANCENDED_UPDATE
...
CREATE PROCEDURE GET_AVG_MISSION_DURATION_BY_HQ_AND_DATE(
    IN id VARCHAR(36),
    IN month INT,
    IN year INT
)
